...
class Restaurant extends CI_Controller {
    ...
    public function view(){
        $data['allRestaurant'] = $this->restaurant_model->getAllRestaurant();
    ...
    public function ajaxview() {
        ...
        foreach ($grid_data['data'] as $key => $value) {
            $edit_active_access = ($this->session->userdata('UserType') == 'MasterAdmin') ? '<button onclick="deleteAll('.$value['content_id'].')"  title="'.$this->lang->line('click_delete').'" class="delete btn btn-sm danger-btn margin-bottom red"><i class="fa fa-times"></i> '.$this->lang->line('delete').'</button>' : '';
    ...
    public function add_menu(){
        ...
        if($this->input->post('submit_page') == "Submit")
        {
          ...
          //  $this->form_validation->set_rules('menu_detail','Detail', 'trim|required');
           // $this->form_validation->set_rules('recipe_detail','Recipe Detail', 'trim|required');
            $this->form_validation->set_rules('recipe_time','Recipe Time', 'trim|required');
            ...
            {  
                ...
                $add_data = array(                  
                    ...
                    'menu_detail' =>$this->input->post('menu_details'),
                    'ingredients' =>$this->input->post('ingredient'),
                    'recipe_detail' =>$this->input->post('recipe_details'),
                ...
                if (!empty($_FILES['Image2']['name']))
                {
                    ...
                    {
                      $img = $this->upload->data();
                      $add_data['image2'] = "menu/".$img['file_name'];   
                    }
                    else
                    {
                      $data['Error'] = $this->upload->display_errors();
                ...
                if (!empty($_FILES['Image3']['name']))
                {
                    ...
                    {
                      $img = $this->upload->data();
                      $add_data['image3'] = "menu/".$img['file_name'];   
                    }
                    else
                    {
                      $data['Error'] = $this->upload->display_errors();
           ...
           // $this->form_validation->set_rules('recipe_detail','Recipe Detail', 'trim|required');
            ...
            {  
                ...
                if (!empty($_FILES['Image2']['name']))
                {
                    ...
                    {
                      $img = $this->upload->data();
                      $edit_data['image2'] = "menu/".$img['file_name'];   
                      if($this->input->post('uploaded_image2')){
                        @unlink(FCPATH.'uploads/'.$this->input->post('uploaded_image2'));
                      } 
                    }
                    else
                    {
                ...
                if (!empty($_FILES['Image3']['name']))
                {
                    ...
                    {
                      $img = $this->upload->data();
                      $edit_data['image3'] = "menu/".$img['file_name'];   
                      if($this->input->post('uploaded_image3')){
                        @unlink(FCPATH.'uploads/'.$this->input->post('uploaded_image3'));
                      } 
                    }
                    else
                    {
    ...
    public function ajaxviewPackage() { 
        ...
        $sortfields = array(3=>'package.created_date',4=>'package.price',5=>'res.name');
        $sortFieldName = '';
        if(array_key_exists($sortCol, $sortfields))
        {
            $sortFieldName = $sortfields[$sortCol];
        ...
        foreach ($grid_data['data'] as $key => $value) {
            ...
            $currency_symbol = $this->common_model->getCurrencySymbol($value['currency_id']);
            $price = ($value['price'])?number_format_unchanged_precision($value['price'],$currency_symbol->currency_code):'';
            $records["aaData"][] = array(
                $nCount,
                $value['name'],
                $currency_symbol->currency_symbol.$price,
                $value['rname'],
                $edit_active_access
            ); 
            ...
            foreach ($Languages as $lang) { 
                if(array_key_exists($lang->language_slug,$value['translations'])){
                    $cusLan[] = '<a href="'.base_url().ADMIN_URL.'/'.$this->controller_name.'/edit_package/'.$lang->language_slug.'/'.str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encryption->encrypt($value['translations'][$lang->language_slug]['translation_id'])).'" title="'.$this->lang->line('click_edit').'"><i class="fa fa-edit"></i> </a>
                    <a style="cursor:pointer;"  onclick="deleteDetail('.$value['translations'][$lang->language_slug]['translation_id'].','.$value['content_id'].')"  title="'.$this->lang->line('click_delete').'"><i class="fa fa-times"></i> </a>
                    ( '.$value['translations'][$lang->language_slug]['name'].' )';
                }else{
                    $cusLan[] = '<a href="'.base_url().ADMIN_URL.'/'.$this->controller_name.'/add_package/'.$lang->language_slug.'/'.$value['content_id'].'" title="'.$this->lang->line('click_add').'"><i class="fa fa-plus"></i></a>';
                }                    
            }
            // added to specific position
            array_splice( $records["aaData"][$cnt], 2, 0, $cusLan);
    ...
    public function getCategory(){
        $entity_id = ($this->input->post('res_id') != '') ? $this->input->post('res_id') : '';
        if ($entity_id) {
            $data['category'] =  $this->restaurant_model->getCategoryShow($entity_id);
            
            header('Content-Type: application/json');
            
            echo json_encode($data['category']);
           
            $this->load->view(ADMIN_URL . '/restaurant_menu', $data['category'] ,true);
    ...
    public function getItem()
    {
        ...
        if ($entity_id) {
            $data['items'] =  $this->restaurant_model->getItemShow($entity_id,$res_id);
            
            header('Content-Type: application/json');
            
            echo json_encode($data['items']);
           
            $this->load->view(ADMIN_URL . '/restaurant_menu', $data['items'] ,true);
    ...
    public function orderUpdate(){
        ...
        if(!empty($ids)){ 
            ...
            foreach ($idArray as $id){ 
                ...
                $update = $this->restaurant_model->updateMenu($data, $id); 
                $count ++;     
            } 
    ...
    public function orderUpdateMenu(){
        $ids = $this->input->post('ids'); 
        ...
        if(!empty($ids)){ 
            ...
            foreach ($idArray as $id){ 
                // Update image order by id 
                $data = array('sort_value' => $count); 
                $update = $this->restaurant_model->updateResMenu($data, $id); 
                $count ++;     
            } 
