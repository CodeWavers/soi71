    ...
    public function login_post()
    {
        ...
        if(!empty($login)){
            ...
            if($login->status==1)
            {
                ...
                $login_detail = array('FirstName'=>$login->first_name,'image'=>$image,'PhoneNumber'=>$login->mobile_number,'UserID'=>$login->entity_id);
                ...
                if(!empty($details)){
                    if($this->post('order_status') == 'cancel'){
                        if($this->post('cancel_reason') != ''){
                            ...
                            $this->driver_api_model->deleteRecord('order_driver_map','order_id',$order_id);
                            //get user of order
