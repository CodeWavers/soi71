...
<div class="pdf_main">
	<div class="head-main">
		...
		<div class="head-right fright">
			...
			<div class="col-li"> <span class="icon"><img src="<?php echo base_url(); ?>/assets/admin/img/calender.png" width="50" alt="" /></span>
				<p>DATE <br>
					<span>
						<?php $date = date("d-m-Y h:i A", strtotime($order_records->order_date));
						echo $date; ?>
					</span>
				</p>
			</div>
		...
		<div class="bill-ship-details">
			<div class="colm fleft">
				...
				if (!empty($user_detail)) { ?>
					<p><?php echo $user_detail['first_name'] . ' ' . $user_detail['last_name'] . '<br>' . $order_records->mobile_number . '<br> Road No: ' . $address->RoadNo . ', Block No: ' . $address->BlockNo . ', Flat No: ' . $address->FlatNo  . ', House No: ' . $address->Houseno . ' , ' . $address->area . '<br>' . $pay ?></p>
				<?php } else { ?>
	...
	<div class="segment-main">
		...
		<div class="div-thead">
			<div>
				...
				<div class="div_3">Custom Fee</div>
			...
			if (!empty($item_detail)) {
				...
				foreach ($item_detail as $key => $value) {
					...
					<div class="b0">
						...
						<div class="center div_3"><?php echo $restaurant_detail->currency_symbol; ?><?php echo ($value['custom_fee']) ? number_format_unchanged_precision($value['custom_fee'], $restaurant_detail->currency_code) : 0 ?></div>
						<div class="center div_4"><?php echo $value['qty_no'] ?></div>
						<div class="center div_5"><?php echo $restaurant_detail->currency_symbol; ?><?php echo ($Subtotal) ? number_format_unchanged_precision(($Subtotal * $value['qty_no']) + $value['custom_fee'] , $restaurant_detail->currency_code) : number_format_unchanged_precision(($price * $value['qty_no']) + $value['custom_fee'], $restaurant_detail->currency_code);
																									$Subtotal = 0;
																									$addons_name_list = '';
																									$vat = ($value['add_vat']) ? $value['add_vat'] + $vat : $vat + 0;
																									$sum = ($Subtotal) ? ($Subtotal * $value['qty_no']) + $value['custom_fee'] + $sum : ($price * $value['qty_no']) + $value['custom_fee'] + $sum;
																									$i = $i + 1;
																									?></div>
					</div>
			<?php }
	...
	<table border="3" cellpadding="10" cellspacing="0" width="100%" class="table-style">
		<tr>
			<td rowspan="8" class="width60"><?php echo ($order_records->extra_comment) ? 'Preferences: ' . $order_records->extra_comment : '' ?></td>
			<td class="align-right" class="width15"><strong>Subtotal</strong></td>
			<td class="align-left" class="width20"><?php echo $restaurant_detail->currency_symbol; ?><?php echo number_format_unchanged_precision($sum, $restaurant_detail->currency_code) ?></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="align-left"><?php echo ($order_records->coupon_amount) ? (($order_records->coupon_type == 'Amount') ? '(-)' . $restaurant_detail->currency_symbol : '') . number_format_unchanged_precision($order_records->coupon_amount, $restaurant_detail->currency_code) : '(-)'; ?><?php echo ($order_records->coupon_type == 'Percentage') ? '% ( ' . number_format_unchanged_precision(($order_records->coupon_amount * $order_records->subtotal) / 100) . ')' : '' ?></td>
		</tr>
		<tr>
			<td class="align-right"><strong>SD</strong></td>
			<td class="align-left"><?php echo $restaurant_detail->currency_symbol; ?><?php echo number_format_unchanged_precision($order_records->sd, $restaurant_detail->currency_code); ?></td>
		</tr>
		<tr>
			<td class="align-right"><strong>VAT</strong></td>
			<td class="align-left"><?php echo $restaurant_detail->currency_symbol; ?><?php echo number_format_unchanged_precision($vat, $restaurant_detail->currency_code); ?></td>
		</tr>
		<!-- <tr>
		...
		<tr>
			<td class="align-right"><strong>ADVANCE PAYMENT</strong></td>
			<td class="align-left"><?php echo $restaurant_detail->currency_symbol; ?><?php echo number_format_unchanged_precision($order_records->advance, $restaurant_detail->currency_code); ?></td>
		</tr>
		<tr>
			<td class="align-right"><strong>DUE</strong></td>
			<td class="align-left"><?php echo $restaurant_detail->currency_symbol; ?><?php echo number_format_unchanged_precision($order_records->due, $restaurant_detail->currency_code); ?></td>
		</tr>
