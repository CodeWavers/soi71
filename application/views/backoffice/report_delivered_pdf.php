...
<!-- <title>Delivered Order Report</title> -->
<style type="text/css">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered">
           ...
           <?php  
                ...
                {  
                  $d1=strtotime($row->time);
                  $d2=strtotime($row->accept_order_time);
                  $duration=round(abs($d1 - $d2) / 60,2). " m";

                 $food_bill= $row->subtotal+$row->coupon_discount;
                 $resto_pay=$row->subtotal+$row->vat+$row->sd-$row->commission_value;
                   ...
                   $totalCusPay+=$row->total_rate;
                    ...
                    echo '<tr>  
                                ...
                                <td>'.$row->e_id.'</td>  
                                <td>'.$duration.'</td>  
                                <td>'.date("d-m-Y H:i:s",strtotime($record->time)).'</td>  
