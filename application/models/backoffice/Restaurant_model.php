...
class Restaurant_model extends CI_Model {
    ...
    public function getMenuGridList($sortFieldName = '', $sortOrder = 'ASC', $displayStart = 0, $displayLength = 10){
       // print_r($this->session->userdata('restaurant'));exit;
        ...
        if($this->input->post('category') != ''){
            $this->db->like('c.name', $this->input->post('category'));
        } 
        $this->db->select('menu.name as mname,res.name as rname,menu.entity_id,menu.status,res.currency_id,c.name as cat_name');
        ...
        if($this->input->post('page_title')=="" && $this->input->post('restaurant') == '' && $this->input->post('price') == '' && $this->input->post('category') == ''){
            $this->db->select('content_general_id,menu.*,res.name as rname,res.currency_id,c.name as cat_name');   
    ...
    public function updateResMenu($data = array(), $id)
    {
        $this->db->where('entity_id', $id);
    ...
    public function getItemShow($id,$resId)
    {
        $this->db->select('name,entity_id');
        $this->db->where('category_id',$id);
        $this->db->where('restaurant_id',$resId);
        $this->db->order_by('sort_value', 'asc');
    ...
    public function getCategoryShow($id){
        ...
        $this->db->distinct();
        $this->db->select('cat.entity_id as id, cat.name as name');
        ...
        // $this->db->order_by('res_menu.sort_value', 'asc');
        return $this->db->get('category as cat')->result();
    }
    
    public function updateRestaurantOrder($data = array(), $id)
    {
        $this->db->where('entity_id', $id);
        $this->db->update('restaurant', $data);
        return $this->db->affected_rows();
