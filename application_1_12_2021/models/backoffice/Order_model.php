...
class Order_model extends CI_Model {
	...
	{
		$this->db->select('u.mobile_number,vat,sd,order.*,res.name, address.address,address.landmark,address.city,address.zipcode,u.first_name,u.last_name,uaddress.address as uaddress,uaddress.landmark as ulandmark,uaddress.city as ucity,uaddress.zipcode as uzipcode');
	...
	public function getCustomizedItems($sortFieldName = '', $sortOrder = 'DESC', $displayStart = 0, $displayLength = 10)
	{
		if($this->input->post('page_title') != ''){
			$this->db->where("CONCAT(u.first_name,' ',u.last_name) like '%".$this->input->post('page_title')."%'");
		}
		if($this->input->post('order') != ''){
			$this->db->like('q.entity_id', $this->input->post('order'));
		}
		
		if($this->input->post('Status') != ''){
			$this->db->like('q.status', $this->input->post('Status'));
		}
		if($this->input->post('restaurant') != ''){           
		...
		if($this->input->post('order_date') != ''){
			$this->db->like('q.date', $this->input->post('order_date'));
		}

		if($this->input->post('phone') != ''){
            $this->db->like('u.mobile_number', $this->input->post('phone'));
        }
		
		$this->db->select('q.*,add.*,u.first_name as fname,u.last_name as lname,u.entity_id as user_id,u.mobile_number,restaurant.name as res_name');
		$this->db->join('users as u','q.user_id = u.entity_id','left');
		$this->db->join('user_address as add','q.address_id = add.entity_id','left');
		$this->db->join('restaurant','q.restaurant_id = restaurant.entity_id','left');
		$this->db->where('q.status',0);
		if($this->session->userdata('UserType') == 'Admin'){
			$this->db->where('restaurant.created_by',$this->session->userdata('UserID'));
		}
		
		$this->db->order_by('q.entity_id','desc');
		$result['total'] = $this->db->count_all_results('quotes as q');
		...
		$this->db->select('q.*,q.entity_id as q_id,add.*,u.first_name as fname,u.last_name as lname,u.entity_id as user_id,u.mobile_number,restaurant.name as res_name');
		$this->db->join('users as u','q.user_id = u.entity_id','left');
		$this->db->join('user_address as add','q.address_id = add.entity_id','left');
		$this->db->join('restaurant','q.restaurant_id = restaurant.entity_id','left');
		if($this->session->userdata('UserType') == 'Admin'){
		...
		$this->db->where('q.status',0);
		$this->db->order_by('q.entity_id','desc');
		$result['data'] = $this->db->get('quotes as q')->result();    
	...
	{
		$this->db->select('q.*,u.first_name as fname,u.last_name as lname,u.entity_id as user_id');
		$this->db->join('users as u','q.user_id = u.entity_id','left');
		//$this->db->join('restaurant','q.restaurant_id = restaurant.entity_id','left');
		$this->db->where('q.entity_id',$order_id);
		return $this->db->get('quotes as q')->result();
	}
	...
	public function getRecord($tblName,$fieldName,$data)
	{
		return $this->db->get_where($tblName, array($fieldName => $data))->first_row();
	}
