...
class Dashboard_model extends CI_Model {
    ...
    public function ajaxCustomNotification(){
        //get last orders
        $this->db->select('quotes.entity_id');
        $this->db->join('restaurant','quotes.restaurant_id = restaurant.entity_id','left');
        if($this->session->userdata('UserType') == 'Admin'){
            $this->db->where('restaurant.created_by',$this->session->userdata('UserID'));
        }
        $this->db->where('DATE(quotes.date)',date('Y-m-d'));
        $this->db->limit(1);
        $this->db->order_by('quotes.entity_id','desc');
        $count = $this->db->get('quotes')->first_row();
        //get notification count
        $this->db->select('last_order_id,order_count,view_status,date');
        $this->db->where('admin_id',$this->session->userdata('UserID'));
        $last_order = $this->db->get('custom_order_notification')->first_row();

        $date = date('Y-m-d');
        if(!empty($count) && (!empty($last_order) && $last_order->last_order_id == 0)){
            $data = array('last_order_id'=>$count->entity_id,'order_count'=>count($count),'date'=>$date);
            $this->db->where('admin_id',$this->session->userdata('UserID'));
            $this->db->update('custom_order_notification',$data);
        }else if(!empty($count) && empty($last_order)){
            $this->db->select('quotes.entity_id as order_count');
            $this->db->join('restaurant','quotes.restaurant_id = restaurant.entity_id','left');
            if($this->session->userdata('UserType') == 'Admin'){
                $this->db->where('restaurant.created_by',$this->session->userdata('UserID'));
            }
            $this->db->where('DATE(quotes.date)',date('Y-m-d'));
            $arrayData = $this->db->get('quotes')->num_rows();
            ...
            $this->db->insert('custom_order_notification',$data);
            $this->db->insert_id();
        }
        if(!empty($count) && !empty($last_order)){
            $order_count = 0;
            ...
            $this->db->where('quotes.entity_id >', $last_order->last_order_id);
            $this->db->where('quotes.entity_id <=', $count->entity_id);
            $this->db->where('DATE(quotes.date)',date('Y-m-d'));
            $arrayData = $this->db->get('quotes')->num_rows();
            if(!empty($arrayData)){
                $order_count = $arrayData;
    ...
    public function changeCustomViewStatus(){
        $data = array('order_count'=>0,'view_status'=>1);
        $this->db->where('admin_id',$this->session->userdata('UserID'));
        $this->db->update('custom_order_notification',$data);
    }	
