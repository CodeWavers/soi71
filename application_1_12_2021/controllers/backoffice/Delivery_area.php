...
class Delivery_area extends CI_Controller { 
    public $controller_name = 'delivery_area';
    public $prefix = 'da';
    public function __construct() {
        ...
        $this->load->model(ADMIN_URL.'/delivery_area_model');
    }
    //view data
    public function view() {
        $data['meta_title'] =' Delivery Area | '.$this->lang->line('site_title');
        $data['Languages'] = $this->common_model->getLanguages();     
        $this->load->view(ADMIN_URL.'/delivery_area',$data);
    }
    //add data
    public function add() {
        $data['meta_title'] =' Delivery Area| '.$this->lang->line('site_title');
        ...
        {
            $this->form_validation->set_rules('name', 'Delivery Area', 'trim|required');
            if ($this->form_validation->run())
            {
                ...
                $add_data = array(                   
                    ...
                    'rider_commision'=>$this->input->post('rider_commision'),
                    'status'=>1,
                ); 
                
                if(empty($data['Error'])){
                    $this->delivery_area_model->addData('delivery_area',$add_data); 
                    $this->session->set_flashdata('page_MSG', $this->lang->line('success_add'));
                    redirect(base_url().ADMIN_URL.'/'.$this->controller_name.'/view');  
                }         
            }
        }
        $this->load->view(ADMIN_URL.'/delivery_area_add',$data);
    }
    //edit data
    public function edit() {
        ...
        if($this->input->post('submit_page') == "Submit")
        {
            ...
            if ($this->form_validation->run())
            {
                ...
                if(empty($data['Error'])){
                    $this->delivery_area_model->updateData($updateData,'delivery_area','entity_id',$this->input->post('entity_id'));
        ...
        $data['edit_records'] = $this->delivery_area_model->getEditDetail($entity_id);
        $this->load->view(ADMIN_URL.'/delivery_area_add',$data);
    }
    //ajax view
    public function ajaxview() {
        ...
        $grid_data = $this->delivery_area_model->getGridList($sortFieldName,$sortOrder,$displayStart,$displayLength);
    ...
    public function ajaxDisable() {
        $entity_id = ($this->input->post('entity_id') != '')?$this->input->post('entity_id'):'';
        if($entity_id != ''){
            $this->delivery_area_model->UpdatedStatus($this->input->post('tblname'),$entity_id,$this->input->post('status'));
        }
    }
    /*
